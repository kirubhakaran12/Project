...
for(cost in c(0.01, 10, 100))
{
  svm_data = data[,c(3,16)]
  svm.model <- svm(svm_data, y=data$Class, kernel="linear", type="C-classification", scale=FALSE, cost = cost)
  prediction <- predict(svm.model, svm_data) 
...
{
  svm_data = data[,c(3,16)]
  svm.model <- svm(svm_data, y=data$Class, kernel="polynomial", degree=deg, type="C-classification")
  prediction <- predict(svm.model, svm_data) 
